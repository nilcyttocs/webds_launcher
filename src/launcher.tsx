...
*/
    ...
    orderedCategories.forEach(cat => {
      ...
      const section = (
        <div className={`jp-NewLauncher-section${_kernel}`} key={cat}>
          <div className="jp-NewLauncher-sectionHeader">
            {webds ? (
...
return (
  <div className="jp-NewLauncher-body">
    <div className="jp-NewLauncher-content">
      <div className="jp-NewLauncher-content-float">{floats}</div>
      <div className="jp-NewLauncher-content-main">{sections}</div>
    </div>
  </div>
);
...
function Card(
  ...
  return (
    <div
      className={`jp-NewLauncher-item${_other}${mode}`}
      title={title}
      onClick={mainOnClick}
      onKeyPress={onkeypress}
      tabIndex={100}
      ...
      key={Private.keyProperty.get(item)}
    >
      <div className={`jp-NewLauncherCard${_other}-icon jp-NewLauncher${mode}-Cell`}>
        <LabIcon.resolveReact
          icon={icon}
          iconClass={classes(iconClass, 'jp-Icon-cover')}
          stylesheet="launcherCard" />
      </div><div
        className={`jp-NewLauncher${_other}-label jp-NewLauncher${mode}-Cell`}
        title={label}
      >
          {label}
      </div>
